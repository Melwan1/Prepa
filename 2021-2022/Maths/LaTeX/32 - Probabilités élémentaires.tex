...
On appelle \Strong{covariance} de $X$ et $Y$, et on note $\cov(X,Y)$ le réel
\begin{center}
...
\begin{remarque}[Remarque (coefficient de corrélation)]{}
...
\end{center}
C'est le \Strong{coefficient de corrélation} de $X$ et $Y$, c'est un réel de l'intervalle $[-1,1]$. Sa valeur absolue est d'autant plus proche de $1$ que $X$ et $Y$ sont dépendantes.
\end{remarque}
...
$\forall \omega \in \Omega, Z(\Omega) = (X(\omega), Y(\omega))$
\end{center}
s'appelle un \Strong{couple de variables aléatoires} sur $\Omega$ (terminologie officielle)
\end{definition}
...
La loi de $Z$ s'appelle la \Strong{loi conjointe} du couple $(X,Y)$. \\

Les lois des variables aléatoires $X$ et $Y$ s'appellent les \Strong{lois marginales} du couple $(X,Y)$. En utilisant le système complet d'événement $(\{Y=y\})_{y \in Y(\Omega)}$, on a, pour tout $x \in X(\omega)$ :
\begin{center}
...
Fixons à présent $y \in Y(\Omega)$, et supposons que $\p(Y=y) \ne 0$. On appelle \strong{loi conditionnelle de $X$ sachant $Y$} la loi de $X$ dans l'espace probabilisé $(\Omega, \p_\{Y=y\})$. Cette loi est donc déterminée par la connaissance, pour tout $x \in X(\Omega)$, de la probabilité $\p_\{Y = y\}(X=x)$. \\
